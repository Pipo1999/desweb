...
foreach ($suits as $suit) {
    foreach ($symbols as $key=>$symbol){
        $cardDeck->addCard(new Card($suit, $symbol,$values[$key]));
    }
}
...
$numberOfCards = 5;
$handPlayer1->add($cardDeck->deal($numberOfCards));
$handPlayer2->add($cardDeck->deal($numberOfCards));

$result = 0;
ob_start();
$cardCollectionWriter->write($cardDeck);
$cards = ob_get_clean();
